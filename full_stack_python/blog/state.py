...
class BlogPostState(SessionState):
    ...
    def get_post_detail(self):
        ...
        with rx.session() as session:
            ...
            result = session.exec(sql_statement
            ).one_or_none()
            self.post = result
            if result is None:
                self.post_content = ""
            ...
            self.post.publish_active = self.post.publish_active
        # return
    ...
    def load_posts(self,*args, **kwargs):
        ...
        with rx.session() as session:
            ...
            ).where(BlogPostModel.userinfo_id == self.
            my_userinfo_id
                )
            ).all()
            self.posts = result
...
class BlogEditFormState(BlogPostState):
    ...
    def publish_display_date(self) -> str:
        # return "2023-12-01" # YYYY-MM-DD formateo!
        if not self.post:
    ...
    def handle_submit(self, form_data):
        ...
        # print(publish_date, publish_time)  # Imprime las fechas ingresadas
